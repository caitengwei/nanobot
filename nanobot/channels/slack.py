...
class SlackChannel(BaseChannel):
    ...
    async def _download_slack_file(self, file_info: dict[str, Any]) -> tuple[str | None, str]:
        ...
        name = Path(raw_name.replace("\\", "/")).name or "slack-file"
        mime = (str(file_info.get("mimetype") or "") or mimetypes.guess_type(name)[0] or "").lower()
        ...
        if size > MAX_ATTACHMENT_BYTES:
            return None, f"[attachment: {name} - too large]"

        media_dir = Path(get_media_dir("slack"))
        media_dir.mkdir(parents=True, exist_ok=True)
        path = media_dir / safe_filename(f"{file_id}_{name}")
        ...
        if mime.startswith("audio/"):
            ...
            if transcription:
                return None, f"[transcription: {transcription}]"
            return None, f"[audio: {path}]"
    ...
    def _looks_like_html_download(response: httpx.Response) -> bool:
        ...
        if "text/html" in content_type:
            return True
        preview = getattr(response, "content", b"")[:256].lstrip().lower()
