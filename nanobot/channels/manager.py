...
class ChannelManager:
    ...
    def _init_channels(self) -> None:
        ...
        transcription_provider = self._resolve_transcription_provider()
        transcription_key = self._resolve_transcription_key(transcription_provider)
        transcription_base = self._resolve_transcription_base(transcription_provider)
        transcription_language = self._resolve_transcription_language()
    ...
    def _resolve_transcription_provider(self) -> str:
        """Pick the configured transcription provider, accepting legacy nested config."""
        channels = self.config.channels
        if "transcription_provider" in getattr(channels, "model_fields_set", set()):
            return channels.transcription_provider
        return (
            getattr(getattr(channels, "transcription", None), "provider", None)
            or channels.transcription_provider
        )

    def _resolve_transcription_key(self, provider: str) -> str:
        """Pick the API key for the configured transcription provider."""
        try:
            nested_key = getattr(self.config.channels.transcription, "api_key", "")
            if nested_key:
                return nested_key
            if provider == "openai":
                return self.config.providers.openai.api_key or ""
            if provider == "qwen3-asr":
                return self.config.providers.dashscope.api_key or ""
    ...
    def _resolve_transcription_base(self, provider: str) -> str:
        ...
        try:
            nested_base = getattr(self.config.channels.transcription, "api_base", "")
            if nested_base:
                return nested_base
            if provider == "openai":
    ...
    def _resolve_transcription_language(self) -> str | None:
        """Pick the optional transcription language hint, accepting legacy nested config."""
        nested_language = getattr(
            getattr(self.config.channels, "transcription", None), "language", None
        )
        return nested_language or self.config.channels.transcription_language
